# ==============================================================================
# REQUESTED ATTRIBUTES SCHEMA
# ==============================================================================
"""
Define the standard attributes requested for every sensor footprint.
This serves as the schema/contract for what attributes we expect to extract.
"""

REQUESTED_ATTRIBUTES = {
    # Required attributes (must be present, even if 'Unknown')
    'required': [
        'sensor',           # Specific satellite/sensor (e.g., 'FF01', 'LG03', 'SPOT6')
        'affiliation',      # Company/agency (e.g., 'Pixxel', 'Maxar', 'Airbus')
        'constellation',    # Satellite constellation (e.g., 'Firefly', 'Legion', 'SPOT')
        'catid',           # Catalog ID / Scene ID / Order ID
        'image_type',      # 'P' (panchromatic) or 'MS' (multispectral)
    ],
    
    # Optional attributes (nice to have)
    'optional': [
        'product_level',   # Processing level (e.g., 'L1C', 'L2A', 'OR1', etc.)
        'resolution',      # Ground sample distance in meters
        'date',           # Acquisition date (datetime)
        'year',           # Acquisition year
        'month',          # Acquisition month
        'day',            # Acquisition day
        'tile_id',        # Scene tile identifier (e.g., 'R1C2')
        'xml_format',     # XML format type for debugging
    ],
    
    # Internal tracking attributes
    'tracking': [
        'confidence',     # Confidence level: 'high', 'medium', 'low', 'none'
        'source_type',    # File extension (e.g., 'TIF', 'JP2')
        'cleaned_filename', # Filename without extension
        'base_image_name',  # Filename without tile info
    ]
}

# Flatten for easy iteration
ALL_REQUESTED_ATTRIBUTES = (
    REQUESTED_ATTRIBUTES['required'] + 
    REQUESTED_ATTRIBUTES['optional'] + 
    REQUESTED_ATTRIBUTES['tracking']
)

# ==============================================================================
# XML TAG MAPPINGS (Sensor-Specific)
# ==============================================================================
"""
Maps XML tag paths to requested attributes for each sensor.
This tells the parser where to find each attribute in the XML.
"""

XML_TAG_MAPPINGS = {
    'Pixxel': {
        'sensor': ['.//Satellite_Id'],  # e.g., "Pixxel-FF01"
        'catid': ['.//Scene_Id', './/Order_Id'],
        'product_level': ['.//Processing_Level', './/Product_Level'],
        'image_type': 'MS',  # Hardcoded - Pixxel is always hyperspectral MS
        'affiliation': 'Pixxel',  # Hardcoded
        'constellation': 'Firefly',  # Hardcoded
    },
    
    'Legion': {
        'sensor': ['.//SATID'],  # e.g., "LG01", "LG02"
        'catid': ['.//CATID'],
        'image_type': ['.//NUMBANDS'],  # Needs logic: 1=P, >1=MS
        'affiliation': 'Maxar',  # Hardcoded
        'constellation': 'Legion',  # Hardcoded
    },
    
    'SPOT': {
        'sensor': ['.//MISSION_INDEX', './/MISSION'],  # e.g., "SPOT 6"
        'catid': ['.//DATASET_NAME', './/JOB_ID'],
        'image_type': ['.//NBANDS'],  # Needs logic: 1=P, >1=MS
        'product_level': ['.//PRODUCT_TYPE', './/PROCESSING_LEVEL'],
        'affiliation': 'Airbus',  # Hardcoded
        'constellation': 'SPOT',  # Hardcoded
    },
    
    'Pleiades': {
        'sensor': ['.//MISSION_INDEX', './/MISSION'],  # e.g., "PHR1A"
        'catid': ['.//DATASET_NAME', './/JOB_ID'],
        'image_type': ['.//NBANDS'],
        'product_level': ['.//PRODUCT_TYPE', './/PROCESSING_LEVEL'],
        'affiliation': 'Airbus',
    ...
    'PleiadesNeo': {
        'sensor': ['.//MISSION_INDEX', './/MISSION'],  # e.g., "PNEO3"
        'catid': ['.//DATASET_NAME', './/JOB_ID'],
        'image_type': ['.//NBANDS'],
        'product_level': ['.//PRODUCT_TYPE', './/PROCESSING_LEVEL'],
        'affiliation': 'Airbus',
    ...
    'Maxar': {
        'sensor': ['.//SATID', './/satelliteId'],  # e.g., "WV02", "GE01"
        'catid': ['.//CATID', './/catalogId'],
        'image_type': ['.//NUMBANDS'],
        'affiliation': 'Maxar',
        # constellation determined by lookup table
    },
}

# ==============================================================================
# STAC/JSON TAG MAPPINGS (For non-XML metadata formats)
# ==============================================================================
"""
Maps JSON paths to requested attributes for sensors using STAC GeoJSON metadata.
"""

STAC_TAG_MAPPINGS = {
    'Satellogic': {
        # JSON paths using dot notation for nested keys
        'sensor': 'properties.satl:product_name',  # e.g., "NewSat-15-Mark-V"
        'constellation': 'properties.constellation',  # "MarkIV" or "MarkV"
        'resolution': 'properties.gsd',  # Ground sample distance
        'product_level': 'properties.satl:product_name',  # Usually "L1D"
        'catid': 'id',  # STAC ID field
        'affiliation': 'Satellogic',  # Hardcoded
        'image_type': 'MS',  # Hardcoded - always MS for Satellogic
    },
}

# ==============================================================================
# METADATA FILE EXTENSIONS BY SENSOR
# ==============================================================================
"""
Maps sensor types to their metadata file extensions.
"""

METADATA_FILE_TYPES = {
    'Pixxel': ['.xml', '.XML'],
    'Legion': ['.xml', '.XML', '.IMD', '.imd'],
    'SPOT': ['.xml', '.XML'],  # DIM_*.XML files
    'Pleiades': ['.xml', '.XML'],  # DIM_*.XML files
    'PleiadesNeo': ['.xml', '.XML'],  # DIM_*.XML files
    'Maxar': ['.xml', '.XML', '.IMD', '.imd'],
    'Satellogic': ['.geojson', '.json', '.GeoJSON', '.JSON'],  # STAC files
}

# ==============================================================================
# FILENAME PARSING PATTERNS (Fallback when XML not available)
# ==============================================================================
"""
Regex patterns to extract attributes directly from filenames.
Used as fallback when XML parsing fails or XML file not found.
"""
...
FILENAME_PATTERNS = {
    'Pixxel': {
        'sensor': r'(FF0[1-6])',
        'catid': r'_(\d{10,15})_',
        'product_level': r'_(L\d[A-Z])\.',
        'date': r'_(\d{8})_',
    },
    
    'Legion': {
        'sensor': r'(LG0[1-6])',
        'catid': r'-(\d{12,15})_\d+_P\d+',
        'date': r'(\d{2}[A-Z]{3}\d{2})',  # YYMMMDD format
        'tile': r'_R(\d+)C(\d+)',
    },
...
IMAGE_TYPE_PATTERNS = {
    'P': ['PAN', 'PANCHROMATIC', '_P_', '_PAN_', 'P2DS','P2AS', 'P3DS'], #'-P3DS', 
    'MS': ['MS', 'MUL', 'MULTI', 'MULTISPECTRAL', '_MS_', '_MUL_', 'M2DS', 'M2AS', 'M3DS',  
           'MS-FS', 'MS-N', 'RGBN', 'RGB', 'NED', '-M3DS']
}